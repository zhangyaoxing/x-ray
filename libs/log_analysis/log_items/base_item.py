...
class BaseItem:
    ...
    def __init__(self, output_folder: str, config):
        ...
        self._output_file = os.path.join(
            output_folder, f"{self.__class__.__name__}.json"
        )
    ...
    def review_results_markdown(self, f):
        ...
        scale = round(
            self._row_count / MAX_DATA_POINTS
            if self._row_count > MAX_DATA_POINTS
            else 1
        )
        ...
        if self._show_scaler:
            ...
            f.write(
                f'<code id="sliderValue_{self.__class__.__name__}">{scale}</code> point(s).*\n\n'
            )
            f.write("</div>\n\n")
        if self._show_reset:
            f.write(
                f'<input type="button" id="reset_{self.__class__.__name__}" value="Reset">\n\n'
            )
        f.write('<script type="text/javascript">\n')
        f.write("document.addEventListener('DOMContentLoaded', function() {\n")
        if self._show_scaler:
            f.write(
                f"let slider = document.getElementById('slider_{self.__class__.__name__}');\n"
            )
            f.write(
                f"let sliderValue = document.getElementById('sliderValue_{self.__class__.__name__}');\n"
            )
        ...
        if self._show_reset:
            f.write(
                f"let resetButton = document.getElementById('reset_{self.__class__.__name__}');\n"
            )
        f.write("let data = [\n")
    ...
    def _write_output(self):
        ...
        with open(self._output_file, "a", encoding="utf-8") as f:
            if self._cache is None:
                self._logger.debug(
                    "Cache is empty, nothing to write for %s", self.__class__.__name__
                )
                return
