...
def discover_nodes(client, parsed_uri):
    ...
    try:
        ...
        if is_master.get("setName"):
            ...
            nodes["members"] = [{
                "host": m["name"], 
                "uri": f"mongodb://{credential}{m['name']}/?authSource={auth_source}&directConnection=true",
            } for m in members]
        else:
            ...
            for k, v in shard_map.items():
                ...
                parsed_map[k] = {
                    "setName": rs_name,
                    "uri": f"mongodb://{credential}{'_'.join(hosts)}/?authSource={auth_source}",
                    "hosts": [{
                        "host": host,
                        "uri": f"mongodb://{credential}{host}/?authSource={auth_source}&directConnection=true"
                    } for host in hosts]
                }
