...
logs = [{"name":"NetworkInterfaceTL","version":"5.0.31"},
...
def test_version_parser():
    target_drivers = ["NetworkInterfaceTL", "mongo-csharp-driver", "mongo-java-driver", "mongocxx", "ext-mongodb:PHP", "mongo-scala-driver", "mongo-go-driver"]
    ...
    for log, target_driver, expected_version in zip(logs, target_drivers, expected_versions):
        ...
        assert parsed_version == Version.parse(expected_version), f"Expected {expected_version}, got {parsed_version} for driver {target_driver}"
...
def test_is_driver_compatible():
    ...
    for log, expected in zip(logs, expected_compatibility):
        ...
        assert is_compatible == expected, f"Expected compatibility {expected} for driver {log['name']}, got {is_compatible}"